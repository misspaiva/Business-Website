    ...
    >
      <nav className="container-editorial flex h-16 items-center justify-between lg:h-18">
        <a
          ...
          aria-label="Paiva Intelligence — home"
        >
        ...
        <div className="hidden items-center gap-9 md:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="group relative text-[13px] font-medium text-ink-muted transition-colors duration-300 hover:text-ink"
            >
              {link.label}
        ...
        <button
          ...
          className="flex h-9 w-9 items-center justify-center text-ink-soft md:hidden"
      ...
      {/* Mobile menu */}
      <div
        className={`overflow-hidden border-t border-surface-line bg-surface/95 backdrop-blur-xl transition-all duration-500 ease-precise md:hidden ${
        ...
        <div className="container-editorial flex flex-col gap-1 py-6">
          {NAV_LINKS.map((link) => (
            <a
              ...
              className="border-b border-surface-line-soft py-3 text-sm font-medium text-ink-soft transition-colors hover:text-ink"
            >
              {link.label}
            </a>
          ))}
